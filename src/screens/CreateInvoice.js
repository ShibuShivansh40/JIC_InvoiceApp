...
const CreateInvoice = ({ navigation, route }) => {
  ...
  const [loadingClients, setLoadingClients] = useState(true);
  const [loadingProducts, setLoadingProducts] = useState(true);
  const [isEditMode, setIsEditMode] = useState(false); // Added state for isEditMode
  ...
  const handleSyncAndPrint = async () => {
    ...
    try {
      let res;
      const encodedRefNo = encodeURIComponent(refNo); // Fix: encode for PUT
      console.log('Encoded refNo for PUT:', encodedRefNo);
